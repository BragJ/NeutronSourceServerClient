...
void TimeOfFlightRunnable::doWork()
{
    ...
    //cout <<"the share_vector tof::::::" << tof <<"    "<< id << endl; 
    ...
    else
    {
		...
		//cout<<"the tof is  ======================" << normal_tof << endl;
            *(p++) = int(normal_tof/NS_TOF_NORM/1000);
        }
    }
    data = freeze(tof);
    cout <<"the share_vector freeze(tof) :::::::::::::" << data <<"    "<< id << endl;       
}
    ...
    while (is_running)
    {
        ...
        if (sleep >= 0)
            epicsThreadSleep(sleep);
             ...
             ++slow;
             //cout << " the slow is ========me ==========" << slow <<endl;
            }
        // Increment the 'ID' of the pulse
        ++id;
        //id += 5; 
        //cout << " the id is ========me ==========" << id <<endl;
        ...
        tof_runnable->createEvents((count+10), id, realistic);
        pixel_runnable->createEvents((count+10), id, realistic);
        ...
        ++packets;
        //packets += 5;
        cout << " the Packet Nums are ===" << packets <<endl;
